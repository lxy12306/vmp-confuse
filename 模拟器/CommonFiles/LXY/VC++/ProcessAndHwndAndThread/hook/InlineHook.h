...
namespace LXY
{
	...
	private:
		...
		bool m_bIsCrossProcess = false;	//是否跨进程
	public:
		/*公有数据成员*/
	...
	public:
		...
		//跨进程的注入
		MyInlineHook32(DWORD dwProcessId, DWORD dwHookBeGin, DWORD dwHookSize,DWORD dwShellCodeBegin,DWORD dwShellCodeSize):m_dwHookBeGin(dwHookBeGin), m_dwHookSize(dwHookSize),m_HookFunc(0),
			dwPid(dwProcessId),dwMyThreadId(GetCurrentThreadId()), m_bIsCrossProcess(true) {
			try
			{
				m_CopyShellCode(dwShellCodeBegin, dwShellCodeSize);
			}
			catch(const LXY::m_WindowsException& me)
			{
				me.e_ShowError();
			}
		}
		~MyInlineHook32() {
			if(!vechThreads.empty())
				m_WakeUpAllThread();
			if (m_bIsCrossProcess)
			{
				HANDLE hProcess = ::OpenProcess(PROCESS_ALL_ACCESS, false, dwPid);
				VirtualFreeEx(hProcess, (LPVOID)m_HookFunc, 0, MEM_RELEASE);
	...
	private:
		...
		//跨进程Hook
		void m_CopyShellCode(DWORD dwShellCodeBegin, DWORD dwShellCodeSize);

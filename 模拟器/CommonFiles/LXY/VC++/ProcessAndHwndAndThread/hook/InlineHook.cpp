#include "F:\Code\object\ProcessAndHwndAndThread\hook\InlineHook.h"
...
namespace LXY
{
	...
	void MyInlineHook32::m_CopyShellCode(DWORD dwShellCodeBegin, DWORD dwShellCodeSize)
	{
		HANDLE hProcess = ::OpenProcess(PROCESS_ALL_ACCESS, false, dwPid);
		if (hProcess == NULL)
			MD_Throw_Windows(MyInlineHook32::m_CopyShellCode::OpenProcess);
		m_HookFunc = (m_func)::VirtualAllocEx(hProcess, NULL, dwShellCodeSize, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
		if (m_lpvAddress == NULL)
			MD_Throw_Windows(MyInlineHook32::m_CopyShellCode::VirtualAllocEx);
		WriteProcessMemory(hProcess, (LPVOID)m_HookFunc, (LPVOID)dwShellCodeBegin, dwShellCodeSize, NULL);//原来的函数
		CloseHandle(hProcess);
	}
